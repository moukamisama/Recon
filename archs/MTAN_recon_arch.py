...
class MTAN_recon(nn.Module):
    def __init__(self, class_nb, tasks, branch_type='no_branch', topK=0, conflict_scores_file=None):
        ...
        super(MTAN_recon, self).__init__()
        ...
        self.encoder_att = nn.ModuleList([self.att_layer([filter[0], filter[0], filter[0]], n_tasks=self.n_tasks)])
        self.decoder_att = nn.ModuleList([self.att_layer([2 * filter[0], filter[0], filter[0]], n_tasks=self.n_tasks)])

        for i in range(4):
            self.encoder_att.append(
                self.att_layer([2 * filter[i + 1], filter[i + 1], filter[i + 1]], n_tasks=self.n_tasks))
            self.decoder_att.append(
                self.att_layer([filter[i + 1] + filter[i], filter[i], filter[i]], n_tasks=self.n_tasks))
        ...
        self.semantic_pred, self.depth_pred, self.normal_pred = None, None, None
    ...
    def task_dependent_modules_names(self):
        return ['encoder_att', 'decoder_att',
                'semantic_pred', 'depth_pred', 'normal_pred']
    ...
    def forward(self, x):
        ...
        for j in range(5):
            if j == 0:
                ...
                atten_encoder[j][1] = dot_recon(atten_encoder[j][0], g_encoder[j][1])
                ...
                atten_encoder[j][0] = self.encoder_att[j](
                    cat_recon(g_encoder[j][0], atten_encoder[j - 1][2], dim=1))
                atten_encoder[j][1] = dot_recon(atten_encoder[j][0], g_encoder[j][1])
                atten_encoder[j][2] = self.encoder_block_att[j](atten_encoder[j][1])
                atten_encoder[j][2] = self.max_pool(atten_encoder[j][2])
        ...
        for j in range(5):
            if j == 0:
                atten_decoder[j][0] = interpolate_recon(atten_encoder[-1][-1], scale_factor=2, mode='bilinear',
                                                     align_corners=True)
                atten_decoder[j][0] = self.decoder_block_att[-j - 1](atten_decoder[j][0])
                atten_decoder[j][1] = self.decoder_att[-j - 1](
                    cat_recon(g_upsampl[j], atten_decoder[j][0], dim=1))
                atten_decoder[j][2] = dot_recon(atten_decoder[j][1], g_decoder[j][-1])

            else:
                atten_decoder[j][0] = interpolate_recon(atten_decoder[j - 1][2], scale_factor=2, mode='bilinear',
                                                     align_corners=True)
                atten_decoder[j][0] = self.decoder_block_att[-j - 1](atten_decoder[j][0])
                atten_decoder[j][1] = self.decoder_att[-j - 1](
                    cat_recon(g_upsampl[j], atten_decoder[j][0], dim=1))
