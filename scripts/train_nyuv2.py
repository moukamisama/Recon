...
def project_args(parser):
    ...
    parser.add_argument('--project', default='NYUv2+MTAN', type=str, help='The name of the project used in wandb')
...
def training_args(parser):
    ...
    parser.add_argument('--bz', default=2, type=int, help='The batch size')
...
def task_args(parser):
    parser.add_argument('--tasks', default=['semantic', 'depth', 'normal'], type=str, nargs='+', help='The name list of tasks')

def model_args(parser):
    parser.add_argument('--arch', default='MTAN_recon', type=str, help='The architecture')
    parser.add_argument('--method', default='Baseline',
                        choices=['Baseline', 'CAGrad', 'Graddrop', 'PCGrad', 'MGD', 'Recon'],
                        type=str, help='The method we used')

def branch_args(parser):
    parser.add_argument('--topK', default=25, type=int, help='The topK layers will be turn into task-specific')
...
def cagrad_args(parser):
    parser.add_argument('--alpha', default=0.4, type=float, help='the alpha used in CAGrad')
...
def task_name_to_metric_map(mode='train'):
    task_name_to_metric_map = {'semantic': [semantic_metric(n_classes=SEG_CLASSES_NYUv2, task='semantic', mode=mode),
                                            losses_metric(task='semantic', mode=mode)],
                               'depth': [depth_metric(task='depth', mode=mode),
                                         losses_metric(task='depth', mode=mode)],
                               'normal': [normal_metric(task='normal', mode=mode),
                                          losses_metric(task='normal', mode=mode)]}
...
if __name__ == '__main__':
    ...
    arch_opt = {'tasks': opt.tasks, 'branch_type': opt.branch_type,
                'topK': opt.topK, 'conflict_scores_file': opt.conflict_scores_file, 'class_nb': SEG_CLASSES_NYUv2}
